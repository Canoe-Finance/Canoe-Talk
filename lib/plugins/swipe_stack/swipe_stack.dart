...
class SwipeStackState extends State<SwipeStack> with SingleTickerProviderStateMixin {
  ...
  void swipeLeft() {
    if (widget.children.isNotEmpty && _animationController.status != AnimationStatus.forward) {
      _animationType = 2;
  ...
  void rewind() {
    if (_history.isNotEmpty && _animationController.status != AnimationStatus.forward) {
      _animationType = 3;
