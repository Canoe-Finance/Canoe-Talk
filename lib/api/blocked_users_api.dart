...
class BlockedUsersApi {
  ...
  Future<List<DocumentSnapshot<Map<String, dynamic>>>> getblockedUsers() async {
    ...
    final QuerySnapshot<Map<String, dynamic>> querySnapshot =
      ...
      logger.info('getBlockedUsers() -> error: ${e.toString()}');
    });
  ...
  Future<bool> blockUser({required String blockedUserId}) async {
    ...
    final query = await (_firestore
        ...
        .catchError((e) {
      // Debug error
      logger.warning('blockUser() -> error: ${e.toString()}');
    });
    ...
    if (query.docs.isEmpty) {
      ...
      // Debug
      logger.info('blockUser() -> success');
      // Result
      return true;
    } else {
      // Debug
      logger.info('blockUser() -> You already blocked this user');
      // Result
      return false;
    }
  }
  ...
  Future<bool> isBlocked({
    ...
    final query = await (_firestore
        ...
        .catchError((e) {
      // Debug error
      logger.warning('isBlocked() -> error: ${e.toString()}');
    });
  ...
  Future<void> deleteBlockedUser(String blockedUserId) async {
    _firestore
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        // Debug
        logger.info('deleteBlock() -> doc does not exists');
      }
    }).catchError((e) {
      // Debug
      logger.warning('deleteBlock() -> error: ${e.toString()}');
    });
  }
  ...
  Future<void> deleteBlockedUsers() async {
    _firestore
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        // Debug
        logger.info('deleteBlockedUsers() -> success');
      }
    });
  }
}
